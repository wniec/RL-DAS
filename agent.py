...
class DQN:
    ...
    def init_parameters(self):
        for param in self.predict_net.parameters():
            stdv = 1.0 / np.sqrt(param.size(-1))
            param.data.uniform_(-stdv, stdv)

    def save(self, log_path, epoch, run_time):
        torch.save(
            self.predict_net.state_dict(),
            os.path.join(log_path, "DQN-" + run_time + f"-{epoch}.pth"),
        )
    ...
    def softmax_explore_policy(self, qvalues):
        ...
        for i in range(qvalues.shape[0]):
            ...
            for j in range(values.shape[1]):
                s += values[i][j]
            actions.append(
                np.random.choice(np.arange(qvalues.shape[1]), p=values[i] / s)
            )
        return actions
    ...
    def replay(self):
        if self.memory.size() >= self.replay_size:
            ...
            Xs, action, next_Xs, reward, is_done = self.memory.n_step_replay(
                self.replay_size
            )
            q_vals = self.predict_q_value(Xs).cpu()

            next_q_vals = self.predict_q_value(next_Xs).cpu()
            q_vals_action = torch.where(
                torch.tensor(is_done),
                torch.tensor(reward),
                torch.tensor(reward)
                + self.gamma * torch.max(next_q_vals, dim=-1).values,
            )

            q_vals.scatter_(
                -1,
                torch.tensor(action, dtype=torch.int64).unsqueeze(1),
                torch.tensor(q_vals_action.unsqueeze(1), dtype=q_vals.dtype),
            )
            q_val_pred = self.forward(Xs)
            qloss = F.mse_loss(q_val_pred, q_vals.to(self.device)).requires_grad_()
            return q_vals, qloss

        return torch.tensor([0.0], requires_grad=True).to(self.device), torch.tensor(
    ...
    def n_step_replay(self):
        if self.memory.size() >= self.replay_size:
            n_step = self.n_steps
            states, actions, next_states, rewards, dones = self.memory.n_step_replay(
                self.replay_size, n_step
            )
            ...
            q_vals.scatter_(
                ...
                torch.tensor(target_q, dtype=q_vals.dtype).unsqueeze(1),
            )
        ...
        for k in range(k_epoch):
            ...
            if old_value is None:
                ...
                vpredclipped = old_value + torch.clamp(
                    bl_val - old_value, -self.eps_clip, self.eps_clip
                )
                v_max = torch.max(
            ...
            if self.max_grad_norm > 0:
                grad_norms = clip_grad_norms(
                    self.optimizer.param_groups, self.max_grad_norm
                )
            self.optimizer.step()
            logger.write(
                "train/Reward", log_steps, {"train/Reward": R.mean().cpu().item()}
            )
            logger.write(
                "train/ratios", log_steps, {"train/ratios": ratios.mean().cpu().item()}
            )
            logger.write(
                "train/baseline_loss",
                log_steps,
                {"train/baseline_loss": baseline_loss.cpu().item()},
            )
            logger.write(
                "train/reinforce_loss",
                log_steps,
                {"train/reinforce_loss": reinforce_loss.cpu().item()},
            )
            ...
            if self.max_grad_norm > 0:
                ...
                logger.write(
                    "train/actor_grad", log_steps, {"train/actor_grad": grad_norms[0]}
                )
                logger.write(
                    "train/critic_grad", log_steps, {"train/critic_grad": grad_norms[1]}
                )
            log_steps += 1
