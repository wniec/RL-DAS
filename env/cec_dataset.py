...
"""
cec2021 = {
    "Bent_cigar": [1, {"bias": 100}],
    "Schwefel": [2, {"bias": 1100}],
    "bi_Rastrigin": [3, {"bias": 700}],
    "Grie_rosen": [4, {"bias": 1900}],
    "Hybrid1": [
        5,
        {
            "length": [0.3, 0.3, 0.4],
...
cec2022 = {
    "Zakharov": [1, {"bias": 300}],
    "Rosenbrock": [2, {"bias": 400}],
    "Escaffer6": [3, {"bias": 600}],
    "Step_Rastrigin": [4, {"bias": 800}],
    "Levy": [5, {"bias": 900}],
    "Hybrid1": [
        6,
        {
            "length": [0.4, 0.4, 0.2],
            "bias": 1800,
            "subproblems": ["Bent_cigar", "Hgbat", "Rastrigin"],
        },
    ],
    "Hybrid2": [
        7,
        {
            "length": [0.1, 0.2, 0.2, 0.2, 0.2, 0.1, 0.2],
            "bias": 2000,
            "subproblems": [
                "Hgbat",
                "Katsuura",
    ...
    "Hybrid3": [
        8,
        {
            "length": [0.3, 0.2, 0.2, 0.1, 0.2],
            "bias": 2200,
            "subproblems": ["Katsuura", "Happycat", "Grie_rosen", "Schwefel", "Ackley"],
        },
    ],
    "Composition1": [
        9,
        {
            "lamda": [1, 1e-6, 1e-6, 1e-6, 1e-6],
            "sigma": [10, 20, 30, 40, 50],
    ...
    "Composition2": [
        10,
        {
            ...
            "bias": [0, 200, 100],
            "F": 2400,
            "subproblems": ["Schwefel", "Rastrigin", "Hgbat"],
        },
    ],
    "Composition3": [
        11,
        {
            "lamda": [1e-6, 10, 1e-6, 10, 5e-6],
    ...
    "Composition4": [
        12,
        {
            "lamda": [10, 10, 2.5, 1e-6, 1e-6, 5e-4],
...
cec2005 = {
    "Composition2005F18": Composition2005F18,
    "Composition2005F23": Composition2005F23,
}
...
class Training_Dataset(Dataset):
    def __init__(
        ...
        if filename is not None and filename != "":
            ...
            if indicated_dataset is None:
                self.data = Training_Dataset.dataset_gen(
                    num_samples,
                    dim,
                ...
                self.data = Training_Dataset.rand_indicated_dataset(
                    indicated_dataset,
                    num_samples,
                    dim,
                    shifted=shifted,
    ...
    def dataset_gen(
        ...
        for i in range(size):
            ...
            if problem == "Composition":
                problem_data = problem_types[problem].generator(
                    filename=None,
                    dim=dim,
                    cf_num=cf_num,
                ...
                problem_data = problem_types[problem].generator(
                    dim, shifted=shifted, rotated=rotated, biased=biased
                )
    ...
    @staticmethod
    def read_for_test(
        problem,  # the name of problem, it should be obtain in benchmark dictionary
        ...
        if problem[:6] == "Hybrid":
            ...
            return eval("Hybrid")(
                ...
                np.array(shift),
                np.array(rotate),
                np.array(length, dtype=int),
                np.array(shuffle),
                np.array(data["bias"]) if biased else 0,
                problem_,
            )
        # Composition problems
            ...
            return eval("Composition")(
                ...
                np.array(data["sigma"]),
                np.array(data["bias"]),
                np.array(data["F"]) if biased else 0,
                problem_,
            )
        # basic problems
        else:
    ...
    def rand_indicated_dataset(
        ...
        for i in range(size):
            ...
            if problem[:11] == "Composition":
                ...
                problem_data = problem_types["Composition"].generator(
                    filename=None,
                    dim=dim,
                ...
                problem_list = dataset[problem][1]["subproblems"]
                problem_data = problem_types["Hybrid"].generator(
                    filename=None,
                    dim=dim,
                    cf_num=len(problem_list),
                    problem_names=problem_list,
...
def Test(
    agent,  # the optimizer object, all objects here should obtain an uniform interface test_run(see optimizer.JDE21)
    ...
    if result_path is None:
        ...
        if indicated_problem is None:
            ...
            result_path += "result_" + (
                indicated_problem
                if isinstance(indicated_problem, str)
                else "+".join(p for p in indicated_problem)
            )
        if indicated_config is not None:
            result_path += "_config" + (
                str(indicated_config)
                if isinstance(indicated_config, int)
                else "".join(str(c) for c in indicated_config)
            )
    ...
    for problem in func_suites.keys():
        ...
        if indicated_problem is not None:
            if (
                isinstance(indicated_problem, list) and problem not in indicated_problem
            ) or (isinstance(indicated_problem, str) and problem != indicated_problem):
                continue
        record = {}
        ...
        for config in range(8):
            if indicated_config is not None:
                if (
                    isinstance(indicated_config, list)
                    and config not in indicated_config
                ) or (isinstance(indicated_config, int) and config != indicated_config):
                    continue
            # initialize records
            ...
            p = Training_Dataset.read_for_test(
                problem, dim, data_path, config, func_suites
            )
            # initialize progress bar
            pbar = tqdm(
                total=30,
                desc=problem + " with config {}".format(config),
...
def random_Test(
    ...
    for i in range(30):
        ...
        problems[i] = problem_types[problem].get_instance(
            problem_types[problem].generator(
                dim=dim, shifted=config & 2, rotated=config & 1, biased=config & 4
            )
        )

    pbar = tqdm(
        total=30, desc="random test", bar_format="{l_bar}{bar:20}{r_bar}{bar:-20b}"
    )
    record = {}
    Fevs = []
    computation_time = []
