...
class Problem:
    ...
    @classmethod
    def generator(
        cls, dim, size=1, shifted=True, rotated=True, biased=True
    ):  # Generate a specified number(size) of instance data of type-assigned problem
        instances = []
    ...
    def store_instance(
        instances, filename
    ):  # Store the problem instance data into a file
        size = len(instances)
        mode = "w"
        for k in range(size):
    ...
    def get_instance(
        cls, problem_data
    ):  # Transfer a batch of instance data to a batch of instance objects
...
class Dif_powers(Problem):
    ...
    def func(self, x):
        ...
        return (
            np.power(
                np.sum(np.power(np.fabs(z), 2 + 4 * i / max(1, self.dim - 1)), -1), 0.5
            )
            + self.bias
        )
...
class Weierstrass(Problem):
    ...
    def func(self, x):
        ...
        for k in range(k_max + 1):
            sum1 += np.sum(
                np.power(a, k) * np.cos(2 * np.pi * np.power(b, k) * (z + 0.5)), -1
            )
...
class Schwefel(Problem):
    ...
    def func(self, x):
        self.FES += x.shape[0]
        z = sr_func(x, self.shift, self.rotate, self.shrink)
        a = 4.209687462275036e002
        b = 4.189828872724338e002
        z += a
        g1 = z * np.sin(np.sqrt(np.abs(z)))
        g2 = (500 - z % 500) * np.sin(np.sqrt(np.fabs(500 - z % 500))) - (
            z - 500
        ) ** 2 / (10000 * self.dim)
        g3 = (-z % 500 - 500) * np.sin(np.sqrt(np.fabs(500 - -z % 500))) - (
            z + 500
        ) ** 2 / (10000 * self.dim)
        g = (
            np.where(np.fabs(z) <= 500, g1, 0)
            + np.where(z > 500, g2, 0)
            + np.where(z < -500, g3, 0)
        )
...
class Katsuura(Problem):
    ...
    def func(self, x):
        ...
        tmp1 = np.repeat(
            np.power(np.ones((1, 32)) * 2, np.arange(1, 33)), x.shape[0], 0
        )
...
class Happycat(Problem):
    ...
    def func(self, x):
        ...
        r2 = np.sum(z**2, -1)
        return (
            np.power(np.fabs(r2 - self.dim), 1 / 4)
            + (0.5 * r2 + sum_z) / self.dim
            + 0.5
            + self.bias
        )
...
class Hgbat(Problem):
    ...
    def func(self, x):
        ...
        r2 = np.sum(z**2, -1)
        return (
            np.sqrt(np.fabs(np.power(r2, 2) - np.power(sum_z, 2)))
            + (0.5 * r2 + sum_z) / self.dim
            + 0.5
            + self.bias
        )
...
class bi_Rastrigin(Problem):
    ...
    def func(self, x):
        ...
        tmp2 = np.minimum(
            np.sum((tmpx - u0) ** 2, -1),
            d * self.dim + s * np.sum((tmpx - u1) ** 2, -1),
        )
...
class Zakharov(Problem):
    ...
    def func(self, x):
        ...
        z = sr_func(x, self.shift, self.rotate, self.shrink)
        return (
            np.sum(np.power(z, 2), -1)
            + np.power(np.sum(0.5 * z, -1), 2)
            + np.power(np.sum(0.5 * z, -1), 4)
        )
...
class Levy(Problem):
    ...
    def func(self, x):
        ...
        _w_ = w[:, :-1]
        return (
            np.power(np.sin(np.pi * w[:, 0]), 2)
            + np.sum(
                np.power(_w_ - 1, 2) * (1 + 10 * np.power(np.sin(np.pi * _w_ - 1), 2))
            )
            + np.power(w[:, -1] - 1, 2)
            * (1 + np.power(np.sin(2 * np.pi * w[:, -1]), 2))
        )
...
class Scaffer_F7(Problem):
    ...
    def func(self, x):
        ...
        return np.power(
            1
            / (self.dim - 1)
            * np.sum(np.sqrt(s) * (np.sin(50 * np.power(s, 0.2)) + 1), -1),
            2,
        )
        ...
        for i in range(x.shape[0]):
            for j in range(self.dim):
                if np.fabs(x[i][j] - self.shift[j]) > 0.5:
                    y[i][j] = (
                        self.shift[j]
                        + np.floor(2 * (x[i][j] - self.shift[j]) + 0.5) / 2
                    )
...
functions = {
    ...
    "Happycat": Happycat,
    "Hgbat": Hgbat,
    "bi_Rastrigin": bi_Rastrigin,
    "Zakharov": Zakharov,
    "Levy": Levy,
    "Scaffer_F7": Scaffer_F7,
...
class Hybrid:
    ...
    @staticmethod
    def read(
        problems_path, size=1
    ):  # Read a specified number of problem data from file
    ...
    @staticmethod
    def generator(
        dim=0,  # problem dimensions, randomly selected if the value <= 0
        ...
        for i in range(size):
            ...
            for i in range(cf_num):
                if (
                    problem_names is None
                ):  # User doesn't assign the problems in the composition
                ...
                while (
                    not indicated_specific and name == "bi_Rastrigin" and length[i] < 5
                ):
            ...
            instances.append(
                ["Hybrid", dim, cf_num, shift, H, length, shuffle, bias, problems]
            )
            mode = "a"
        return (
            instances if size > 1 else instances[0]
        )  # for convenience, return a single object if the size is 1
    ...
    def read(
        ...
        with open(problems_path, "r") as fpt:
            ...
            for i in range(size):
                ...
                instances.append(
                    ["Composition", dim, cf_num, lamda, sigma, bias, F, problems]
                )
        return instances
    ...
    def func(self, x):  # evaluate a group of solutions
        ...
        for j in range(x.shape[0]):
            for i in range(self.cf_num):
                a = np.sqrt(
                    np.sum(
                        (x[j][: self.sub_problems[i].dim] - self.sub_problems[i].shift)
                        ** 2
                    )
                )
                if a != 0:
                    w[j][i] = (
                        ...
                        * np.exp(
                            -np.sum(
                                (
                                    x[j][: self.sub_problems[i].dim]
                                    - self.sub_problems[i].shift
                                )
                                ** 2
                            )
                            / (
                                2
                                * self.sub_problems[i].dim
                                * self.sigma[i]
                                * self.sigma[i]
                            )
                        )
            ...
            for i in range(cf_num):
                ...
                problems.append(
                    functions[name].generator(
                        dim, shifted=shifted, rotated=rotated, biased=False
                    )
                )
            # if user chooses to store problem set into a file
            if store:
...
class Composition2005:
    def __init__(
        self, dim, cf_num, sub_problems, lamda, sigma, bias, C, F, func_data, M
    ):
        self.dim = dim
        self.cf_num = cf_num
        self.lamda = lamda
        ...
        for i in range(self.cf_num):
            self.fmax[i] = np.fabs(
                self.sub_problems[i].func(
                    rotatefunc(
                        np.ones((1, self.dim)) * 5 / self.lamda[i], self.rotates[i]
                    )
                )[0]
            )
        self.FES = 0
    ...
    def func(self, x):  # evaluate a group of solutions
        ...
        for j in range(x.shape[0]):
            for i in range(self.cf_num):
                ...
                w[j][i] = np.exp(
                    -np.sum(a / (2 * self.dim * self.sigma[i] * self.sigma[i]))
                )
            mid = np.argmax(w[j])
            sum_w = np.sum(w[j])
            for i in range(self.cf_num):
...
class Composition2005F18:
    def __init__(self, dim):
        ...
        zero = np.zeros(self.dim)
        eye = np.eye(self.dim)
        self.sub_problems.append(Ackley(dim, zero, eye, 0))
        self.sub_problems.append(Ackley(dim, zero, eye, 0))
        self.sub_problems.append(Rastrigin(dim, zero, eye, 0))
        self.sub_problems.append(Rastrigin(dim, zero, eye, 0))
        self.sub_problems.append(Sphere(dim, zero, eye, 0))
        self.sub_problems.append(Sphere(dim, zero, eye, 0))
        self.sub_problems.append(Weierstrass(dim, zero, eye, 0))
        self.sub_problems.append(Weierstrass(dim, zero, eye, 0))
        self.sub_problems.append(Griewank(dim, zero, eye, 0))
        self.sub_problems.append(Griewank(dim, zero, eye, 0))
        for i in range(self.cf_num):
            self.sub_problems[i].shrink = 1
        self.fmax = np.zeros(self.cf_num)
